...
for_vars <- c()
x = 1
OutVals <- boxplot(df.num1[[numeric_variables[x]]], plot = F)$out
...
for(o in for_vars){
  OutVals <- boxplot(df.num1[[o]], plot = F)$out
  mean <- mean(df.num1[[o]], na.rm = T)
  
  o1 <- ifelse(OutVals < mean, OutVals, NA) %>% na.omit()
  o3 <- ifelse(OutVals > mean, OutVals, NA) %>% na.omit()
  
  val1 <- quantile(df.num1[[o]], 0.25, na.rm = T) -
    1.5 * IQR(df.num1[[o]], na.rm = T)
  df.num1[which(df.num1[[o]] %in% o1), o] <- val1
  
  val3 <- quantile(df.num1[[o]], 0.75, na.rm = T) +
    1.5 * IQR(df.num1[[o]], na.rm = T)
  df.num1[which(df.num1[[o]] %in% o3), o] <- val3
}



#One-hot encoding
df.chr1 <- dummyVars(" ~ .", data = df.chr1) %>% 
  predict(newdata = df.chr1) %>% 
  as.data.frame()

df <- cbind(df.chr1,df.num1) %>%
  select(cty,everything())
