...
df3 <- fread("C:/Users/lenovo/Downloads/Data science/Bootcamp/R/Week 10/Day 2/crimes.csv")
df3 %>% glimpse()
...
df3 %>% select(variables) %>% inspect_na()

df3 <- df3 %>% select(variables)
...
for_vars <- c()
x = 1
OutVals <- boxplot(df3.num[[numeric_variables[x]]], plot = F)$out

if(length(OutVals > 0)){
  for_vars[x] <- numeric_variables[x]
}


for_vars <- c()
for(x in 1:length(numeric_variables)){
  OutVals <- boxplot(df3.num[[numeric_variables[x]]], plot = F)$out
  if(length(OutVals)> 0){
    for_vars[x] <- numeric_variables[x]
  }
...
for(o in for_vars){
  OutVals <- boxplot(df3.num[[o]], plot = F)$out
  mean <- mean(df3.num[[o]], na.rm = T)
  
  o1 <- ifelse(OutVals < mean, OutVals, NA) %>% na.omit()
  o3 <- ifelse(OutVals > mean, OutVals, NA) %>% na.omit()
  
  val1 <- quantile(df3.num[[o]], 0.25, na.rm = T) -
    1.5 * IQR(df3.num[[o]], na.rm = T)
  df3.num[which(df3.num[[o]] %in% o1), o] <- val1
  
  val3 <- quantile(df3.num[[o]], 0.75, na.rm = T) +
    1.5 * IQR(df3.num[[o]], na.rm = T)
  df3.num[which(df3.num[[o]] %in% o3), o] <- val3
}
...
features <- df3 %>% select(-ViolentCrimesPerPop) %>% names()
...
df3 <- df3 %>% select(ViolentCrimesPerPop,features)


# Standardize (Normalize) ----
df3 %>% glimpse()

predictors <- df3[,-1]; target <- df3[,1]
predictors <- predictors %>% scale() %>% as.data.frame()
df3 <-cbind(target,predictors)
